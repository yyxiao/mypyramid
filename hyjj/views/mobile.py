...
from ..common.constant import CODE_ERROR, CODE_SUCCESS, CODE_WRONG, SMS_DESC
...
class MobileView(BaseUtil):
    ...
    def send_code(self):
        ...
        if not error_msg:
            code = make_random(6)
            content = SMS_DESC % (user_name, code)
            redis_host = self.registry.settings['redis.sessions.host']
            r = create_redis(redis_host)
            num = r.get(self.client_addr)
            num = int(num) if num else 0
            if num <= 9:
                add_code_redis(user_phone, code, redis_host)
                add_ip_no_redis(self.client_addr, num + 1, redis_host)
        ...
        HyLog.log_in(self.client_addr, '', ('sendCode failed ' + error_msg if error_msg else 'sendCode success'),
                     'mobile')
    ...
    def account_binding(self):
        ...
        user_phone = self.POST.get('phone', '')
        user_name = self.POST.get('name', '')
        verification_code = self.POST.get('verificationCode', '')
        wechat_id = self.POST.get('wechatId', '')
        is_risk = 0
        if not user_phone:
            error_msg = '用户手机不能为空！'
        elif not user_name:
        ...
        HyLog.log_in(self.client_addr, '', ('accountBinding failed ' + error_msg if error_msg
